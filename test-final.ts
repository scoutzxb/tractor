...
let state = createTrumpState(false, DEALER);
...
for (const seat of seats) {
  ...
  if (levelCards.length >= 1) {
    ...
    state = declare(state, seat, [card], LEVEL, DEALER);
    declarationMade = true;
    
    console.log(`📣 ${seat} 亮主: ${SUIT_NAMES[card.suit!]}${LEVEL} (${SUIT_NAMES[card.suit!]}主，优先级7)`);
...
const toDiscard = smartDiscardKitty(dealerHand, ctx, 6);
...
const newDealerHand = dealerHand.filter(c => !discardIds.has(c.id));
hands[seats.indexOf(DEALER)] = newDealerHand;
  ...
  for (let i = 0; i < seats.length; i++) {
    ...
    for (const cards of possibleChaos) {
      if (canChaoDi(state, seat, cards, LEVEL)) {
        ...
        const newHand = chaoDiHand.filter(c => !returnIds.has(c.id));
        hands[seats.indexOf(seat)] = newHand;
...
console.log(`✅ 亮主牌型: ${state.currentTrump?.cards.map(c => 
  c.joker ? (c.joker === 'big' ? '大王' : '小王') : `${SUIT_NAMES[c.suit!]}${c.rank}`
).join(' ') || '无'}\n`);
...
for (const seat of seats) {
  ...
  ['spade', 'heart', 'club', 'diamond'].forEach(suit => {
    const cards = suitCards.get(suit as any);
    if (cards && cards.length > 0) {
      console.log(`  【${SUIT_NAMES[suit as any]}】 ${cards.map(c => `${SUIT_NAMES[c.suit!]}${c.rank}`).join(' ')}`);
    }
  });
