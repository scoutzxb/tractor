...
import { analyzeDeclaration, TRUMP_PRIORITY } from './src/core/trump-state';
...
function analyzeHandForTrump(hand: Card[], level: Rank): { priority: number; suit: Suit | null; cards: Card[]; desc: string }[] {
...
function testCorrectChaoDi() {
  ...
  for (const seat of seats) {
    ...
    if (declarations.length > 0) {
      ...
      declarations.forEach(d => {
        console.log(`    ✓ ${d.desc} (${d.suit ? SUIT_NAMES[d.suit] + '主' : '无主'}，优先级${d.priority})`);
      });
    } else {
      console.log('  可亮的主: (无)');
    }
  ...
  // 阶段2: 初始亮主
  console.log('\n📦 阶段2: 初始亮主阶段\n');
  console.log('规则: 按顺序亮主，优先级高者胜出，但庄家不变\n');
  ...
  for (const seat of seats) {
    ...
    // 必须比当前高
    if (currentDeclaration && best.priority >= currentDeclaration.priority) continue;
  ...
  for (let round = 0; round < 3; round++) {
    ...
    for (const seat of seats) {
      ...
      for (const d of declarations) {
        if (d.priority >= chaoDiDeclaration.priority) continue;
