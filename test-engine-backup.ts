...
// 创建亮主状态机
let state = createTrumpState(LEVEL, DEALER);
  ...
  for (const seat of seats) {
    ...
    for (const decl of declarations) {
      ...
      if (canChaoDi(state, seat, decl.cards, LEVEL)) {
        ...
        const trumpSuit = state.currentTrump?.suit;
        console.log(`   ✅ 炒底成功！新主花色: ${trumpSuit ? SUIT_NAMES[trumpSuit] : '无主'}`);
        console.log(`   ${seat} 获得底牌并需要扣回6张牌\n`);
        
        // 模拟扣回底牌
        hands[seats.indexOf(seat)].push(...toDiscard);
        // 简化：随机扣回6张
        hands[seats.indexOf(seat)] = hands[seats.indexOf(seat)].slice(6);
